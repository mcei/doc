Контекстом называют экземпляр типа `Context` из пакета `context`, который входит в стандартную библиотеку Go (начиная с версии 1.7).
...
Контекст используется для того, чтобы передавать метаданные запросов, устанавливать таймауты и делать явную отмену в цепочке вложенных функций или горутин.
...
Принято передавать контекст явным образом в виде первого параметра функции и использовать имя переменной `ctx`.
...
Новые контексты создаются на основе родительских. То есть мы добавляем в контекст информацию, оборачивая имеющийся контекст в одну из функций, возвращающую новый контекст. Это позволяет использовать контексты для передачи в глубокие слои кода.
...
```
Здесь функция `context.WithTimeout` принимает два параметра: существующий контекст и `time.Duration`.
...
Метод `Deadline` экземпляра типа `context.Context` помогает узнать в какой момент времени произойдет отмена. Метод возвращает `time.Time` и `ok`. `Deadline` возвращает `ok=false`, когда deadline не установлен.
...
В пакете имеется функция `WithValue` для добавления пары ключ-значение в контекст. Она принимает три параметра: родительский контекст, ключ и значение.
