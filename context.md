...
Пример создания корневых контекстов:
...
Корневые контексты могут создаваться в любом месте программы, но обычно они создаются в main функции.

По своей сути оба корневых контекста ничем не отличаются, `но context.TODO()` предполагается использовать только на время проектирования или эксперимента. Например, когда в одной из функций нужно вызывать внешний API и передавать контекст, но в эту функцию не передается родительский контекст.

В этом случае допускается в первой реализации создать `context.TODO()` внутри самой функции и далее передавать его как родительский. При этом обычно подразумевается, что в следующей реализации в функцию будет проброшен корректный родительский контекст.
...
То есть мы добавляем в контекст информацию, оборачивая имеющийся контекст в одну из функций, возвращающую новый контекст. Это позволяет использовать контексты с различной функциональностью для передачи в глубокие слои кода.
