...
### Как valueCtx находит значение по ключу

Если ключ не существует в текущем контексте, то поиск будет рекурсивно продолжаться по дереву контекстов пока не будет найден корневой контекст. Если значение не найдено, то возвращается nil.
  
К плюсам такой реализации можно отнести тот факт, что поиск значения происходит во всех вложенных контекстах, что значительно упрощает его получение. При этом неважно насколько глубоко вложенным является контекст с искомым значением.

К минусам можно отнести то, что значение контекста может быть изменено в каком-либо промежуточном слое, но при этом мы получим значение из самого ближнего слоя. Также стоит отметить, что при большой вложенности линейный перебор связного списка контекстов может быть довольно затратным.
