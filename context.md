...
Автоматическая отмена контекста по времени можно сделать при помощи функций `WithDeadline` и `WithTimeout`. Они позволяют ограничить максимальное время выполнения блока кода.
