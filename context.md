...
По своей сути оба корневых контекста ничем не отличаются, но `context.TODO()` предполагается использовать только на время проектирования или эксперимента. Например, когда в каком-либо месте программы нужно вызывать внешний API и передать контекст, но ранее не было запроектировано передавать сюда родительский контекст.

В этом случае допускается в первой реализации создать `context.TODO()` внутри функции и далее передавать его как родительский. При этом обычно подразумевается, что в следующей реализации в функцию будет проброшен корректный родительский контекст.
...
Преимущество такой реализации в том, что предполагается переиспользование глобального закрытого канала внутри пакета. 
